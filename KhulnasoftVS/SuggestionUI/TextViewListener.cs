...
internal class KhulnasoftCompletionHandler : IOleCommandTarget, IDisposable
{
    private readonly KhulnasoftVSPackage package;
    ...
    private void OnSuggestionAccepted(String proposalId)
    {
        ...
        ThreadHelper.JoinableTaskFactory
            .RunAsync(async delegate {
                await KhulnasoftVSPackage.Instance.LogAsync($"Accepted completion {proposalId}");
                await KhulnasoftVSPackage.Instance.LanguageServer.AcceptCompletionAsync(proposalId);
            })
            .FireAndForget(true);
    }
    ...
    internal KhulnasoftCompletionHandler(IVsTextView textViewAdapter, ITextView view,
        TextViewListener provider)
    {
        try
        {
            KhulnasoftVSPackage.EnsurePackageLoaded();
            ...
            if (_document != null)
            {
                KhulnasoftVSPackage.Instance.LogAsync("KhulnasoftCompletionHandler filepath = " + _document.FilePath);
            ...
            _ = Task.Run(() =>
            {
                try
                {
                    CompleteSuggestionCommand = GetCommandsAsync("KhulnasoftAcceptCompletion").Result;
                }
                catch (Exception e)
                {
                    Debug.Write(e);
    ...
    private void CaretUpdate(object sender, CaretPositionChangedEventArgs e)
    {
        ...
        {
            ThreadHelper.JoinableTaskFactory
                .RunAsync(async delegate { await KhulnasoftVSPackage.Instance.LogAsync(ex.ToString()); })
                .FireAndForget(true);
        }
    }
    ...
    void ShowIntellicodeMsg()
    {
        if (IsIntellicodeEnabled())
        {
            VsShellUtilities.ShowMessageBox(
                this.package,
                "Please disable IntelliCode to use Khulnasoft. You can access Intellicode settings via Tools --> Options --> Intellicode.",
                "Disable IntelliCode",
...
internal class TextViewListener : IVsTextViewCreationListener
{
    ...
    public void VsTextViewCreated(IVsTextView textViewAdapter)
    {
        ...
        Func<KhulnasoftCompletionHandler> createCommandHandler = delegate()
        {
            return new KhulnasoftCompletionHandler(textViewAdapter, textView, this);
        };
        textView.TextBuffer.Properties.GetOrCreateSingletonProperty<KhulnasoftCompletionHandler>(typeof(KhulnasoftCompletionHandler), createCommandHandler);
    }
}
}
