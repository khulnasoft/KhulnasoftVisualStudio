...
[Guid(PackageGuids.KhulnasoftVSString)]
[InstalledProductRegistration(Vsix.Name, Vsix.Description, Vsix.Version)]
[PackageRegistration(UseManagedResourcesOnly = true, AllowsBackgroundLoading = true)]
[ProvideMenuResource("Menus.ctmenu", 1)]
[ProvideOptionPage(typeof(SettingsPage), "Khulnasoft", "Khulnasoft", 0, 0, true)]
[ProvideToolWindow(
...
public sealed class KhulnasoftVSPackage : ToolkitPackage
{
    internal static KhulnasoftVSPackage? Instance { get; private set; }
                                                  ...
                                                  IProgress<ServiceProgressData> progress)
    {
        ...
        {
            await LogAsync(
                $"KhulnasoftVSPackage.InitializeAsync: Failed to get settings page, using the default settings");
        ...
        {
            await LogAsync(
                $"KhulnasoftVSPackage.InitializeAsync: Failed to register commands; Exception {ex}");
            await VS.MessageBox.ShowErrorAsync(
                "Khulnasoft: Failed to register commands.",
                "Khulnasoft might not work correctly. Please check the output window for more details.");
        }
        ...
        {
            await LogAsync("Khulnasoft Error" + ex);
            throw;
        }

        await LanguageServer.InitializeAsync();
        await LogAsync("Khulnasoft Extension for Visual Studio");
    }
    ...
    public static async Task EnsurePackageLoadedAsync()
    {
        ...
        Guid guidPackage = new(PackageGuids.KhulnasoftVSString);
    ...
    public async Task UpdateSignedInStateAsync()
    {
        ...
        if ((await GetServiceAsync(typeof(IMenuCommandService)))is OleMenuCommandService cmdService)
        {
            MenuCommand? commandSignIn =
                cmdService.FindCommand(new CommandID(PackageGuids.KhulnasoftVS, PackageIds.SignIn));
            MenuCommand? commandSignOut =
                cmdService.FindCommand(new CommandID(PackageGuids.KhulnasoftVS, PackageIds.SignOut));
            MenuCommand? commandEnterToken = cmdService.FindCommand(
                new CommandID(PackageGuids.KhulnasoftVS, PackageIds.EnterAuthToken));
        ...
        if (!IsSignedIn())
        {
            ...
            NotificationAuth.Show("[Khulnasoft] To enable Khulnasoft, please sign in to your account",
    ...
    public static void OpenInBrowser(string url)
    {
        ...
        Instance?.Log($"Khulnasoft failed to open the browser, please use this URL instead: {url}");
        VS.MessageBox.Show(
            "Khulnasoft: Failed to open browser",
    ...
    public string GetAppDataPath()
    {
        ...
        return Path.Combine(appData, ".khulnasoft");
    }
    ...
    public string GetAPIKeyPath() { return Path.Combine(GetAppDataPath(), "khulnasoft_api_key"); }
