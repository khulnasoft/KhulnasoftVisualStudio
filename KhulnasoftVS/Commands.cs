...
namespace KhulnasoftVS.Commands;
...
internal sealed class CommandOpenChatWindow : BaseCommand<CommandOpenChatWindow>
{
    ...
    {
        ToolWindowPane toolWindowPane = await KhulnasoftVSPackage.Instance.ShowToolWindowAsync(
            typeof(ChatToolWindow), 0, create: true, KhulnasoftVSPackage.Instance.DisposalToken);
        if (toolWindowPane == null || toolWindowPane.Frame == null)
        {
            throw new NotSupportedException("Cannot create Khulnasoft chat tool window");
        }
    }
}
...
internal sealed class CommandSignIn : BaseCommand<CommandSignIn>
{
    protected override async Task ExecuteAsync(OleMenuCmdEventArgs e)
    {
        await KhulnasoftVSPackage.Instance.LanguageServer.SignInAsync();
    }
}
...
internal sealed class CommandSignOut : BaseCommand<CommandSignOut>
{
    protected override async Task ExecuteAsync(OleMenuCmdEventArgs e)
    {
        await KhulnasoftVSPackage.Instance.LanguageServer.SignOutAsync();
    }
}
    ...
    where T : class, new()
{
    ...
    protected static KhulnasoftCompletionHandler? completionHandler;
    protected override void BeforeQueryStatus(EventArgs e)
    {
        ...
        ThreadHelper.JoinableTaskFactory.Run(async delegate
        {
            ...
            try
            {
                ...
                {
                    completionHandler = props.GetProperty<KhulnasoftCompletionHandler>(key);
                    return true;
                }
    ...
    where T : class, new()
{
    ...
    protected async Task<FunctionInfo?> GetFunctionInfoAsync()
    {
        ...
        if (func == null)
        {
            await KhulnasoftVSPackage.Instance.LogAsync("Error: Could not get function info");
            return null;
        }
...
internal class CommandExplainCodeBlock : BaseCommandContextMenu<CommandExplainCodeBlock>
{
    protected override void BeforeQueryStatus(EventArgs e)
    {
        ...
        if (Command.Visible)
            Command.Text =
                is_function ? "Khulnasoft: Explain Function" : "Khulnasoft: Explain Code block";
    }
...
internal class CommandRefactorCodeBlock : BaseCommandContextMenu<CommandRefactorCodeBlock>
{
    protected override void BeforeQueryStatus(EventArgs e)
    {
        ...
        if (Command.Visible)
            Command.Text =
                is_function ? "Khulnasoft: Refactor Function" : "Khulnasoft: Refactor Code block";
    }
    ...
    where T : class, new()
{
    ...
    protected async Task<bool> ResolveCodeBlock(int startLine)
    {
        ...
        IList<Packets.ClassInfo>? classes = await KhulnasoftVSPackage.Instance.LanguageServer.GetClassInfosAsync(
            docView.FilePath,
            text,
            languageInfo,
            0,
    ...
    protected CodeBlockInfo ClassToCodeBlock(ClassInfo c)
    {
        ...
        catch (Exception ex)
        {
            Task.Run(async () =>
            {
                return KhulnasoftVSPackage.Instance.LogAsync(ex.ToString());
            });
        }
...
internal class CommandRefactorSelectionCodeBlock : BaseCommandCodeLens<CommandRefactorSelectionCodeBlock>
{
    protected override async Task ExecuteAsync(OleMenuCmdEventArgs e)
    {
        ...
        try
        {
            ...
            if (e.InValue != null)
            {
                await KhulnasoftVSPackage.Instance.LogAsync(e.InValue.ToString());
        ...
        {
            await KhulnasoftVSPackage.Instance.LogAsync(ex.ToString());
        }
    }
}
