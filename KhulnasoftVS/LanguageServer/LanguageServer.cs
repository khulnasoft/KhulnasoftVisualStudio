...
public class LanguageServer
{
    ...
    private string _languageServerVersion = "1.28.3";
    ...
    private readonly KhulnasoftVSPackage _package;
    ...
    {
        _package = KhulnasoftVSPackage.Instance;
        _metadata = new();
    ...
    public async Task SignInWithAuthTokenAsync(string authToken)
    {
        ...
        if (_metadata.api_key == null)
        {
            ...
            var msgboxResult = await VS.MessageBox.ShowAsync(
                "Khulnasoft: Failed to sign in. Please check the output window for more details.",
    ...
    public async Task SignInAsync()
    {
        ...
        async Task<string?> WaitForAuthTokenAsync()
        {
            ...
            if (result == null)
            {
                // show an error message box
                var msgboxResult = await VS.MessageBox.ShowAsync(
                    "Khulnasoft: Failed to get the Authentication Token. Please check the output window for more details.",
                    "Do you want to retry?",
                    OLEMSGICON.OLEMSGICON_WARNING,
                    OLEMSGBUTTON.OLEMSGBUTTON_RETRYCANCEL,
                    OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST);
        ...
        KhulnasoftVSPackage.OpenInBrowser(url);
    ...
    private async Task GetLanguageServerInfoAsync()
    {
        string extensionBaseUrl =
            (_package.SettingsPage.ExtensionBaseUrl.Equals("")
                 ? "https://github.com/khulnasoft/khulnasoft-release/releases/download"
        ...
        if (_package.SettingsPage.EnterpriseMode)
        {
            ...
            catch (Exception)
            {
                ...
                extensionBaseUrl = "https://github.com/khulnasoft/khulnasoft-release/releases/download";
            }
        }
                                                          ...
                                                          IVsThreadedWaitDialog4 progressDialog)
    {
        ...
        progressDialog.UpdateProgress(
            ...
            $"Khulnasoft: Downloading language server v{_languageServerVersion} ({e.ProgressPercentage}%)",
            (int)e.BytesReceived,
                                                       ...
                                                       string downloadDest)
    {
        ...
        progressDialog.StartWaitDialog("Khulnasoft",
                                       $"Extracting files...",
                                       "Almost done",
                                       null,
                                       $"Khulnasoft: Extracting files...",
                                       0,
                                       false,
                                       true);
        ...
        if (e.Error != null)
        {
            ...
            errorBar.Show(
                "[Khulnasoft] Critical Error: Failed to download the language server. Do you want to retry?",
    ...
    public async Task PrepareAsync()
    {
        ...
        progressDialog.StartWaitDialog(
            ...
            "",
            null,
            $"Khulnasoft: Downloading language server v{_languageServerVersion}",
            0,
            false,
            true);
    ...
    private async Task<bool> VerifyLanguageServerSignatureAsync()
    {
        ...
        await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        errorBar.Show(
            "[Khulnasoft] Failed to verify the language server digital signature. The executable might be corrupted.",
    ...
    private async Task StartAsync(bool ignoreDigitalSignature = false)
    {
        ...
        string apiUrl = (_package.SettingsPage.ApiUrl.Equals("") ? "https://server.khulnasoft.com"
                                                                 : _package.SettingsPage.ApiUrl);
        ...
        catch (Exception ex)
        {
            ...
            new NotificationInfoBar().Show(
                "[Khulnasoft] Critical error: Failed to create language server directories. Please check the output window for more details.",
                KnownMonikers.StatusError,
                true,
                null,
        ...
        catch (Exception ex)
        {
            ...
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
            errorBar.Show(
                "[Khulnasoft] Critical Error: Failed to start the language server. Do you want to retry?",
                KnownMonikers.StatusError,
                true,
                null,
                [.. actions, .. NotificationInfoBar.SupportActions]);
        ...
        catch (Exception ex)
        {
            ...
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
            new NotificationInfoBar().Show(
                "[Khulnasoft] Failed to read output from the language server, Khulnasoft might not work properly.",
                KnownMonikers.IntellisenseWarning,
                true,
                null,
                NotificationInfoBar.SupportActions);
            ...
            {
                new NotificationInfoBar().Show(
                    "[Khulnasoft] Critical Error: Failed to get the language server port. Please check the output window for more details.",
                    KnownMonikers.StatusError,
                    true,
                    null,
                    NotificationInfoBar.SupportActions);
