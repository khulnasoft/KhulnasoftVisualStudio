...
{
    readonly KhulnasoftVSPackage Package;
    public WebSocket? ws = null;
    public LanguageServerController() { Package = KhulnasoftVSPackage.Instance; }
    ...
    private void ApplyDiff(string filePath, int start_line, int end_line, string replacement)
    {
        ThreadHelper.JoinableTaskFactory
            .RunAsync(async delegate {
                ...
                // we'd have to wait for the `KhulnasoftInlineDiffViewProvider` to run
...
internal static class WebChatServer
{
    internal static WebServerRequest NewRequest()
    {
        ...
        WebServerRequest request = new() { get_chat_message_request = new() {
            ...
            metadata = KhulnasoftVSPackage.Instance?.LanguageServer.GetMetadata(),
            prompt = ""
        } };
