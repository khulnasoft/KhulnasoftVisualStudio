...
namespace KhulnasoftVS.QuickInfo;

internal sealed class KhulnasoftAsyncQuickInfoSource
(ITextBuffer textBuffer)
    : PropertyOwnerExtension<ITextBuffer, KhulnasoftAsyncQuickInfoSource>(textBuffer),
      IAsyncQuickInfoSource, IDisposable
{
                                                           ...
                                                           CancellationToken cancellationToken)
    {
        ...
        Assumes.True(_tagAggregator != null,
                     "Khulnasoft Quick Info Source couldn't create a tag aggregator for error tags");
        ...
        {
            Debug.Fail("The Khulnasoft Quick Info Source is being called when it shouldn't be.");
            return null;
        }
        ...
        if (appToSpan != null && problemMessage.Length > 0)
        {
            var hyperLink = ClassifiedTextElement.CreateHyperlink(
                "Khulnasoft: Explain Problem",
                "Ask khulnasoft to explain the problem",
                () =>
                {
                    ThreadHelper.JoinableTaskFactory
                        .RunAsync(async delegate {
                            // TODO: Has the package been loaded at this point?
                            await KhulnasoftVSPackage.Instance.LanguageServer.Controller
                                .ExplainProblemAsync(
    ...
    private async Task GetTagAggregatorAsync(ITextView textView)
    {
        ...
        else if (_tagAggregatorTextView != textView)
        {
            throw new ArgumentException(
                "The Khulnasoft Quick Info Source cannot be shared between TextViews.");
        }
    }
}

[Export(typeof(IAsyncQuickInfoSourceProvider))]
[Name("Khulnasoft Quick Info Provider")]
[ContentType("any")]
...
internal sealed class AsyncQuickInfoSourceProvider : IAsyncQuickInfoSourceProvider
{
    ...
    {
        return KhulnasoftAsyncQuickInfoSource.GetOrCreate(
            textBuffer, () => new KhulnasoftAsyncQuickInfoSource(textBuffer));
    }
}
